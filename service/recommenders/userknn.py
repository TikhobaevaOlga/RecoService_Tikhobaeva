...
class UserKnn:
    ...
    def __init__(self, model: ItemItemRecommender = TFIDFRecommender(), N_users: int = 20):
    ...
    def online_recommend(self, user_id, N_recs: int = 10):
        if not self.is_fitted:
            raise ValueError("Please call fit before get recommendations")
    ...
    def offline_recommend(self, ready_recos, user_id):
        try:
            user_knn_recs = ready_recos[str(user_id)]
            return user_knn_recs
        except KeyError:
            return None
